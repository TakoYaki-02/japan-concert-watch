...
- 初回検出から40日以内の公演に `NEW` を表示（初期取り込み分は対象外）
- アーティスト名・地域・情報元で絞り込み（既定地域は関東3県：東京・千葉・神奈川）
- 選択中の年月に関係なく、全期間の `NEW` だけを表示
...
- APIキーや認証情報を必要としない
- HTMLの `noindex` と `robots.txt` で検索エンジンのクロールを拒否する（アクセス制限そのものではない）
...
8. CLUB QUATTRO
9. K-POPコンサートナビ（K-POP・アジア圏の日本公演を横断取得）
10. K-POP MUSIC NOW LIVE（K-POP・アジア圏の年間公演一覧）
